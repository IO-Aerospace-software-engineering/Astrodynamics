...
public class ZenithAttitudeTests
{
    ...
    void Create()
    {
        ...
        ZenithAttitude zenithAttitude = new ZenithAttitude(TestHelpers.EarthAtJ2000,Time.MinValue, TimeSpan.FromHours(1.0), eng);
        Assert.Equal(Time.MinValue, zenithAttitude.MinimumEpoch);
    ...
    public void Execute()
    {
        ...
        ZenithAttitude maneuver = new ZenithAttitude(TestHelpers.EarthAtJ2000,Time.MinValue, TimeSpan.Zero, spc.Engines.First());
        maneuver.TryExecute(orbitalParams.ToStateVector());

        Assert.Equal(new StateOrientation(new Quaternion(0.7071067811865476, 0.0, 0.0, -0.7071067811865476), Vector3.Zero, Time.J2000, Frames.Frame.ICRF),
            maneuver.StateOrientation);
        Assert.Equal(0.0, maneuver.FuelBurned);
        Assert.Equal(new Window(Time.J2000, TimeSpan.Zero), maneuver.ManeuverWindow);
        Assert.Equal(new Window(Time.J2000, TimeSpan.Zero), maneuver.ThrustWindow);
    }
}