...
{
    auto force = IO::Astrodynamics::Integrators::Forces::ComputeForce(3.986004418e14 / IO::Astrodynamics::Constants::G, 10.0, 7000000, IO::Astrodynamics::Math::Vector3D(1, 0, 0));
    ASSERT_EQ(IO::Astrodynamics::Math::Vector3D(-81.347028938775509, 0, 0), force);
}
...
TEST(GravityForce, ApplyToBody)
{
    ...
    ASSERT_EQ(IO::Astrodynamics::Math::Vector3D(-3986.0043543609595, 0, 0), force);
}
...
TEST(GravityForce, ApplyToBodyWithSatellites)
{
    ...
    ASSERT_EQ(IO::Astrodynamics::Math::Vector3D(-8620.5686852713916, -3.59325822164271, -1.3815220770404948), force);
}