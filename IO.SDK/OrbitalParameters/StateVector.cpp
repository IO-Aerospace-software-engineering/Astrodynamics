...
IO::SDK::OrbitalParameters::StateVector IO::SDK::OrbitalParameters::StateVector::CheckAndUpdateCenterOfMotion() const
{
	...
	while (currentBody->GetOrbitalParametersAtEpoch())
	{
		...
		//Compute force
		force = IO::SDK::Integrators::Forces::ComputeForce(currentBody->GetOrbitalParametersAtEpoch()->GetCenterOfMotion()->GetMass(), 1.0, position.Magnitude(), position.Normalize());
